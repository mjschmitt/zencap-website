...
async function cleanupInsights() {
  try {
    ...
    const response = await fetch('http://localhost:3001/api/insights?admin=true');
    ...
    // Step 3: Delete duplicates and extras using SLUG (not ID)
    console.log('🗑️ Deleting duplicate and extra insights...');
    for (const insight of toDelete) {
      try {
        const deleteResponse = await fetch('http://localhost:3001/api/insights', {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ slug: insight.slug })
        });
        ...
        if (deleteResponse.ok) {
          ...
          console.log(`   ❌ Failed to delete: ${insight.title} - ${errorText}`);
        }
    ...
    // Step 4: Add the 12 new enhanced insights with correct field names
